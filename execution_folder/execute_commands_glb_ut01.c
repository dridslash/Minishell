...
/*   Updated: 2022/04/11 14:19:24 by mnaqqad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			int *pipes, int iterate_for_fds, t_env **env_var)
{
	...
	{
		ft_pwd_separat_send_redi(holder_nodes, pipes, iterate_for_fds, env_var);
	}
	...
	int *pipes, int iterate_for_fds, t_env **env_var)
{
	...
	else if (ft_strcmp(which_built_in(holder_nodes), "env") == 0)
	{
		...
		separat_send_redi_two(holder_nodes, pipes, iterate_for_fds, env_var);
}
